...
cdef class DistanceMetric:
    ...
    **Metrics intended for two-dimensional vector spaces:**  Note that the haversine
    ...
    'greatcircle' GreatCircleDistance   6371.009*atan2(sqrt((cos(x2)*sin(y2-y1))^2 +
...
# fun fact: pi / 180 == 0.017453292519943295
cdef inline double radians(double degrees) nogil:
...
cdef class GreatCircleDistance(DistanceMetric):
    """Great Circle (spherical Earth) Distance

    The great circle distance is the physical distance, in kilometers, two
    ...
    cdef inline DTYPE_t rdist(self, DTYPE_t* x1, DTYPE_t* x2,
        ...
        if size != 2:
            with gil:
                raise ValueError("Great circle distance only valid "
                                 "in 2 dimensions")
        cdef DTYPE_t lat1 = radians(x1[0])
        cdef DTYPE_t lng1 = radians(x1[1])
        cdef DTYPE_t lat2 = radians(x2[0])
        cdef DTYPE_t lng2 = radians(x2[1])
        cdef DTYPE_t sin_lat1 = sin(lat1)
        cdef DTYPE_t cos_lat1 = cos(lat1)
        cdef DTYPE_t sin_lat2 = sin(lat2)
        cdef DTYPE_t cos_lat2 = cos(lat2)

        cdef DTYPE_t delta_lng = lng2 - lng1
        cdef DTYPE_t sin_delta_lng = sin(delta_lng)
        cdef DTYPE_t cos_delta_lng = cos(delta_lng)

        cdef DTYPE_t d = atan2(sqrt((cos_lat2 * sin_delta_lng) ** 2 +
                   (cos_lat1 * sin_lat2 -
