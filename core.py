...
def check_geography(N, lat_long, cutoff):
    # fun fact: 6371.009 * pi, approx 20015 km, is farthest apart two points can be on earth.
    if cutoff <= 0 or cutoff >= 20015:
        raise CutoffError('Please provide a cutoff in km.')
    if lat_long.shape != (N, 2):
        raise GeographyError("Your lat_long variable should be an N-by-2 array. The first column is latitude, the second is longitude, both measured in degrees. Rows represent observations.")
    if not np.isfinite(lat_long).all():
        raise GeographyError("Your lat_long variable must be fully populated with real numbers. The first column is latitude, the second is longitude, both measured in degrees.")
    ...
    if lat_abs_max <= np.pi / 2 and long_max <= np.pi and long_min > -np.pi:
        #warnings.warn("Your lat_long variable should be measured in degrees, but it looks like you've provided radians – all the latitudes are in [-π/2, π/2] and the longitudes are in (-π, π].", UserWarning)
...
def check_parameters_panel(y, X, time, lat_long, cutoff):
    ...
    if time.shape[0] != nobs:
        raise ValueError("Your time array must have the same number of rows as your X and y arrays.")
...
def cross_section(formula_like, data, lat_long, cutoff, kernel = 'uniform'):
    ...
    lag_long = parse_lat_long(lat_long, data)
...
def parse_lat_long(lat_long, data):
    ...
    if (len(lat_long) == 2 and isinstance(lat_long[0], str) and
        ...
        try:
            ...
            " data, but failed. Please provide a valid column name or simply pass an " +
            "array.".format(lat_colname, long_colname))
...
def panel(formula_like, data, lat_long, time, group, dist_cutoff, time_cutoff = None,
    ...
    if isinstance(time, str):
        try:
            ...
            " Please provide a valid column name or simply pass an array.".
            format(time))
            raise KeyError(error_str)
    lag_long = parse_lat_long(lat_long, data)
    ...
    if dist_kernel == 'uniform':
        ...
        neighbors, neighbor_distances = query_radius_cached(lat_long, r = dist_cutoff, return_distance = True)
        filling = multiply_XeeX(neighbors, residuals, X, dist_kernel, distances = neighbor_distances, cutoff = dist_cutoff)
        del neighbor_distances
    del balltree, neighbors, y, residuals
