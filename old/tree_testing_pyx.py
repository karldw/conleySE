...
#from distance cimport great_circle
import feather
...
cdef double[:, :] great_circle_one_to_many(double[:, :] latlong_array, double[:] latlong_point):
    cdef int N = latlong_array.shape[0]
    cdef double[:, :] dists = np.empty((N, 1))
    cdef int i
    cdef double[2,] latlong_one_pt
...
def conley_basic(y, X, lat_long, cutoff, kernel_fn = always_one):
    ...
    assert X.shape[0] == N
    cdef double[:, :] bread = np.linalg.inv(X.T @ X)
    # I have no idea if this leaf_size is reasonable.  If running out of memory, divide N by a larger number.
    # 40 is the default.
    cdef int leaf_size = max(40, N // 1000)
    cdef double[:, :] betahat =  bread @ X.T @ y  # '@' is matrix multiplication, equivalent to np.dot or __mul__ on matrices
    #betahat = sm.OLS(Y,X).fit().params  # probably better
    cdef double[:, :] residuals = y - X @ betahat
    # type?
    sklearn_balltree = BallTree(lat_long, metric = 'greatcircle', leaf_size = leaf_size)
    # neighbors is an array of arrays of integers
    # neighbor_distances is an array of array of doubles
    neighbors, neighbor_distances = sklearn_balltree.query_radius(lat_long, r = cutoff, return_distance = True)
    del sklearn_balltree
    cdef double[:, :] meat_matrix = np.zeros((k, k))

    # Types for everything that appears in the loop
    cdef int i, j
    cdef double[:,] neighbor_idx,
    cdef double[:, :] X_i
    cdef double e_i, e_j, kernel_value
    cdef double[:, :] one_pair_mat
    for i, neighbor_idx in enumerate(neighbors):
        assert len(neighbor_idx) >= 1  # sanity check
        e_i = float(residuals[i, ])
        X_i = X[i, ].reshape(-1, 1)
        for j in neighbor_idx:
            e_j = float(residuals[j, ])
            X_j = X[j, ].reshape(-1, 1)
            ...
            meat_matrix = np.add(meat_matrix, one_pair_mat)
    meat_matrix = np.divide(meat_matrix, N)
    cdef double[:, :] sandwich = N * (bread.T @ meat_matrix @ bread)
    cdef double[:, :] se = sqrt(np.diag(sandwich)).reshape(-1, 1)
...
def conley_full_loop(y, X, lat, lon, cutoff, metric = 'sol'):
    ...
    cdef double[:, :] residuals = y - X @ betahat
    cdef double[:, :] meat_matrix = np.zeros((k, k))
    cdef double[:, :] row_of_ones = np.ones((1, N))
    cdef double[:, :] column_of_ones = np.ones((k, 1))
    cdef int loop_count = 0
    cdef double[:, :] latlong_combined = np.hstack((lat.reshape(-1, 1), lon.reshape(-1, 1)))
    cdef int i
    cdef double lonscale, latscale
    cdef double[:, :] dist, X_i, XeeXh
    cdef int[:, :] window

    for i in range(N):
        lonscale = np.cos(lat[i] * 3.1415926 / 180.0) * 111.0
        latscale = 111.0
        ...
        XeeXh = ((X_i @ row_of_ones * residuals_i) * (column_of_ones @ np.multiply(residuals.T, window.T))) @ X
                    # k x 1       1 x n        1 x 1             k x 1                1 x n        n x k
        meat_matrix = np.add(meat_matrix, XeeXh)
    meat_matrix = np.divide(meat_matrix, N)
    cdef double[:, :] sandwich = N * (bread.T @ meat_matrix @ bread)
    cdef double[:, :] se = sqrt(np.diag(sandwich)).reshape(-1, 1)
    cdef double[:, :] results = np.hstack((betahat, se))
    return results
